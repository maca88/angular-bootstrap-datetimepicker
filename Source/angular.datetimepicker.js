                ...
                compile: function (tInput, tAttrs) {
                    ...
                    return {
                        pre: function (scope, input, attrs, controller) {
                            ...
                            if (attrs.bsIcon !== undefined || attrs.bsClearIcon !== undefined) {
                                ...
                                if (tAttrs.bsIcon !== undefined && tAttrs.bsClearIcon !== undefined) {
                                    ...
                                    rightIcon += '<span class="input-group-addon" data-action="clear">' + '<span class="glyphicon glyphicon-' + clearIcon + '" ></span>' + '</span>';
                                } else {
                                    rightIcon = '<span class="input-group-addon" ' + (tAttrs.bsClearIcon !== undefined ? 'data-action="clear"' : '') + ' >' + '<span class="glyphicon glyphicon-' + (tAttrs.bsClearIcon !== undefined ? clearIcon : dateIcon) + '" ></span>' + '</span>';
                            ...
                            dpElem.on('change.dp', function (e) {
                        ...
                        post: function (scope, input, attrs, controller) {
                            ...
                            if (angular.isString(settings.daysOfWeekDisabled))
                                settings.daysOfWeekDisabled = scope.$eval(settings.daysOfWeekDisabled);
                            ...
                            if (controller) {
                                ...
                                controller.$parsers.unshift(function (viewValue) {
                                    ...
                                    if (!parsedDate || !parsedDate.isValid()) {
                                        ...
                                        var isValid = dtInstance && dtInstance.isValid(parsedDate);
                                ...
                                controller.$render = function () {
                                    if (dtInstance)
                                        dtInstance.date(controller.$viewValue !== undefined ? controller.$viewValue : null);
                                };
                                ...
                                if (tAttrs.dpEnabledDates) {
                                    scope.$watch(tAttrs.dpEnabledDates, function (val) {
                                        if (dtInstance)
                                ...
                                //watch for disabledDates changes
                                if (tAttrs.dpDaysOfWeekDisabled) {
                                    scope.$watch(tAttrs.dpDaysOfWeekDisabled, function (val) {
                                        if (dtInstance)
                                            dtInstance.daysOfWeekDisabled(val);
                                    });
                                }
                            }

                            var disabledPicker = function () {
                                if (!dtInstance)
                                    return;
                                dtInstance.disable();
                                angular.element('[data-action]', dpElem).addClass('disabled');
                            };
                            ...
                            var enablePicker = function () {
                                if (!dtInstance)
                                    return;
                                dtInstance.enable();
                                angular.element('[data-action]', dpElem).removeClass('disabled');
                            };
                            ...
                            $timeout(function () {
                                ...
                                if (fieldset.length && fieldset.attr('ng-disabled')) {
                                    scope.$watch(fieldset.attr('ng-disabled'), function (newVal) {
                                        if (newVal === undefined)
                                            return;
                                        newVal ? disabledPicker() : enablePicker();
                                    });
